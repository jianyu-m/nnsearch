...
grid_size_lat = (max_lat - min_lat) / n
grid_size_long = (max_long - min_long) / n
grid_size = max(grid_size_lat, gird_size_long)
...
def grid_search(grid_records, lat, long, k_set):
  for (r <- grid_record):
    if r < k_set.max or len(k_set) < k
      k_set.insert(r)
  rerurn k_set
\end{minted}
\caption{Single grid search.}
\end{listing}
